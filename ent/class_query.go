...
func (cq *ClassQuery) QueryInstructors() *StaffQuery {
	query := &StaffQuery{config: cq.config}
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := cq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (cq *ClassQuery) Clone() *ClassQuery {
	...
	return &ClassQuery{
		...
		withInstructors: cq.withInstructors.Clone(),
...
func (cq *ClassQuery) WithInstructors(opts ...func(*StaffQuery)) *ClassQuery {
	query := &StaffQuery{config: cq.config}
	for _, opt := range opts {
		opt(query)
	}
	cq.withInstructors = query
	return cq
}
...
func (cq *ClassQuery) sqlAll(ctx context.Context) ([]*Class, error) {
	...
	if query := cq.withInstructors; query != nil {
		fks := make([]driver.Value, 0, len(nodes))
		ids := make(map[uuid.UUID]*Class, len(nodes))
		for _, node := range nodes {
			ids[node.ID] = node
			fks = append(fks, node.ID)
			node.Edges.Instructors = []*Staff{}
		}
		var (
			edgeids []uuid.UUID
			edges   = make(map[uuid.UUID][]*Class)
		...
		_spec := &sqlgraph.EdgeQuerySpec{
			...
			Predicate: func(s *sql.Selector) {
				s.Where(sql.InValues(class.InstructorsPrimaryKey[0], fks...))
			},
			ScanValues: func() [2]interface{} {
				return [2]interface{}{new(uuid.UUID), new(uuid.UUID)}
			},
		...
		if err := sqlgraph.QueryEdges(ctx, cq.driver, _spec); err != nil {
			return nil, fmt.Errorf(`query edges "instructors": %w`, err)
		}
		query.Where(staff.IDIn(edgeids...))
		neighbors, err := query.All(ctx)
		if err != nil {
			return nil, err
		}
		for _, n := range neighbors {
			nodes, ok := edges[n.ID]
			if !ok {
				return nil, fmt.Errorf(`unexpected "instructors" node returned %v`, n.ID)
			}
			for i := range nodes {
				nodes[i].Edges.Instructors = append(nodes[i].Edges.Instructors, n)
			}
		}
	}
